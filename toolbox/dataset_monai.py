...
class MonaiDatasetConfig:
    ...
    gen_train_transforms_func: Optional[
        Callable[[str], Compose]
    ] = None  # input argument of the function is sequence type
    gen_val_transforms_func: Optional[
        Callable[[str], Compose]
    ] = None  # input argument of the function is sequence type
...
class RSNA_MICCAI_MONAI_Dataset(pl.LightningDataModule):
    ...
    def _gen_subject_data_list_from_csv(
        self, mode: str
        ...
        mgmt_value_type = int if mode == "train" else float
        df = pd.read_csv(
            os.path.join(self.config.dataset_dir, csv_file),
            dtype={"BraTS21ID": str, "MGMT_value": mgmt_value_type},
        )
        subject_dict_list = []
            ...
            subject_dict_list.append(
                {
                    ...
                    C.T1GD: os.path.join(
                        self.config.dataset_dir, mode, subject_id, C.T1GD
                    ),
                    C.FLAIR: os.path.join(
                        self.config.dataset_dir, mode, subject_id, C.FLAIR
                    ),
    ...
    def setup(self, stage):
        if stage == "fit" or stage is None:
            self.train_data_list, self.val_data_list = partition_dataset(
                self.source_train_data_list,
                ratios=[
                    self.config.train_val_split_ratio,
                    (1 - self.config.train_val_split_ratio),
                ],
                shuffle=True,
        ...
        if stage == "test" or stage is None:
            self.test_ds = Dataset(
                data=self.test_data_list, transform=self.val_transforms
            )
